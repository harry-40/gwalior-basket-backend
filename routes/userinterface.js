...
router.post("/fetch_products_by_category", function (req, res, next) {
  ...
  pool.query(
    "select PL.*, (select P.pricetype from products P where P.productid=PL.productid) as pricetype, (select P.productname from products P where P.productid=PL.productid) as productname,(select P.image from products P where P.productid=PL.productid) as productimage from listproduct PL where PL.categoryid=(select categoryid from categories where categoryname=?)",
